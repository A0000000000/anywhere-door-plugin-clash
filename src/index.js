...
async function processCommand(raw, extend) {
    ...
    if (cmds.length >= 1) {
        if (cmds[0] in cmdList) {
            return await cmdList[cmds[0]](cmds, extend)
        } else {
...
function main() {
    ...
    const clashPath = process.env.CLASH_PATH || ''
    const clashToken = process.env.CLASH_TOKEN || ''
    if (clashPath === '' || clashToken === '') {
        console.error('clash path or clash token is null')
        return
    }
